...
describe('getService', () => {
    describe('getDefaultPatterns', () => {
        ...
        it('should throw a non-Error object if reading the file fails', () => {
            mockReadFileSync.mockImplementation(() => { throw 'File not found'; });
            expect(() => getService.getDefaultPatterns(false)).toThrow();
        });
    });
    ...
    describe('getPatternsFromPath', () => {
        ...
        it('should get patterns from a URL', async () => {
            ...
            mockParse.mockReturnValue(['feature/.*', 'bugfix/.*']);
            const patterns = await getService.getPatternsFromPath('https://example.com/patterns.yml', false);
            expect(patterns).toEqual(['feature/.*', 'bugfix/.*']);
        });
        ...
        it('should get patterns from a URL with wildcard', async () => {
            ...
            const patterns = await getService.getPatternsFromPath('https://example.com/patterns.yml', true);
            expect(patterns).toEqual(['^feature/.*$', '^bugfix/.*$']);
        });
        ...
        it('should throw an error if fetching from a URL fails', async () => {
            ...
            await expect(getService.getPatternsFromPath('https://example.com/patterns.yml', false)).rejects.toThrow();
        });

        it('should throw an error if reading the file fails', async () => {
            mockReadFileSync.mockImplementation(() => { throw new Error('File not found'); });
            await expect(getService.getPatternsFromPath('some/path', false)).rejects.toThrow();
        });
